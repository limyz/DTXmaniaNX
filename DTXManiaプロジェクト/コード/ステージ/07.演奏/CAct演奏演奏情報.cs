...
namespace DTXMania
{
	internal class CAct演奏演奏情報 : CActivity
	{
		...
		public void t進行描画( int x, int y )
		{
			...
			{
                    y += 0x143;
                    CDTXMania.act文字コンソール.tPrint(x, y, C文字コンソール.Eフォント種別.白, string.Format("BGM/D/G/B Adj: {0:####0}/{1:####0}/{2:####0}/{3:####0} ms", CDTXMania.DTX.nBGMAdjust, CDTXMania.ConfigDB.nInputAdjustTimeMs.Drums, CDTXMania.ConfigDB.nInputAdjustTimeMs.Guitar, CDTXMania.ConfigDB.nInputAdjustTimeMs.Bass));
                    y -= 0x10;
                    CDTXMania.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "BGMAdjCommon : {0:####0} ms", CDTXMania.ConfigDB.nCommonBGMAdjustMs ) );
                    y -= 0x10;
                    ...
                    if (CDTXMania.ConfigDB.nSoundDeviceType != 0)
                    {
