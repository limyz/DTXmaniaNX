...
namespace DTXMania
{
    internal class CActSelectQuickConfig : CActSelectPopupMenu
    {
        ...
        private List<CItemBase> MakeListCItemBase(int nConfigSet, int nInst)
        {
            ...
            l.Add(new CItemInteger("ScrollSpeed", 0, 1999, CDTXMania.ConfigDB.n譜面スクロール速度[nInst],
                "演奏時のドラム譜面のスクロールの\n" +
                "速度を指定します。\n" +
            ...
            l.Add(new CItemList("Dark", CItemBase.Eパネル種別.通常, (int)CDTXMania.ConfigDB.eDark,
                "HALF: 背景、レーン、ゲージが表示\n" +
                "されなくなります。\n" +
                "FULL: さらに小節線、拍線、判定ラ\n" +
                "イン、パッドも表示されなくなります。",
            ...
            l.Add(new CItemInteger("Risky", 0, 10, CDTXMania.ConfigDB.nRisky,
                "Riskyモードの設定:\n" +
                "1以上の値にすると、その回数分の\n" +
            ...
            l.Add(new CItemInteger("PlaySpeed", 5, 40, CDTXMania.ConfigDB.n演奏速度,
                "曲の演奏速度を、速くしたり遅くした\n" +
                "りすることができます。\n" +
                "（※一部のサウンドカードでは正しく\n" +
                "再生できない可能性があります。）",
            ...
            #endregion
            #region [ 個別 Sud/Hid ]
            l.Add(new CItemList("HID/SUD", CItemBase.Eパネル種別.通常, CDTXMania.ConfigDB.nHidSud[nInst],
                "",
                "",
            ...
            #endregion
            #region [ 個別 Ghost ]
            l.Add( new CItemList("AUTO Ghost", CItemBase.Eパネル種別.通常, (int)CDTXMania.ConfigDB.eAutoGhost[ nInst ],
                "AUTOプレーのゴーストを指定します。\n",
                "Specify Play Ghost data.\n",
                new string[] {"Perfect", "Last Play", "Hi Skill", "Hi Score", "Online" }
                ));
            l.Add(new CItemList("Target Ghost", CItemBase.Eパネル種別.通常, (int)CDTXMania.ConfigDB.eTargetGhost[ nInst ],
                "ターゲットゴーストを指定します。\n",
        ...
        private List<int> tConfigureAuto_DefaultSettings()
        {
            ...
            else if (CDTXMania.ConfigDB.bAutoPlay.LC == false && CDTXMania.ConfigDB.bAutoPlay.HH == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BD == false && CDTXMania.ConfigDB.bAutoPlay.SD == false &&
                        CDTXMania.ConfigDB.bAutoPlay.HT == false && CDTXMania.ConfigDB.bAutoPlay.LT == false &&
                        CDTXMania.ConfigDB.bAutoPlay.FT == false && CDTXMania.ConfigDB.bAutoPlay.CY == false &&
                        CDTXMania.ConfigDB.bAutoPlay.LP == true && CDTXMania.ConfigDB.bAutoPlay.LBD == true)
            {
                automode = 1;	// Auto LP
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.LC == false && CDTXMania.ConfigDB.bAutoPlay.HH == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BD == true && CDTXMania.ConfigDB.bAutoPlay.SD == false &&
                        CDTXMania.ConfigDB.bAutoPlay.HT == false && CDTXMania.ConfigDB.bAutoPlay.LT == false &&
                        CDTXMania.ConfigDB.bAutoPlay.FT == false && CDTXMania.ConfigDB.bAutoPlay.CY == false &&
                        CDTXMania.ConfigDB.bAutoPlay.LP == false && CDTXMania.ConfigDB.bAutoPlay.LBD == false)
            {
                automode = 2;	// Auto BD
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.LC == false && CDTXMania.ConfigDB.bAutoPlay.HH == false &&
            ...
            else if (CDTXMania.ConfigDB.bAutoPlay.LC == true && CDTXMania.ConfigDB.bAutoPlay.HH == true &&
                        CDTXMania.ConfigDB.bAutoPlay.BD == false && CDTXMania.ConfigDB.bAutoPlay.SD == false &&
                        CDTXMania.ConfigDB.bAutoPlay.HT == false && CDTXMania.ConfigDB.bAutoPlay.LT == false &&
                        CDTXMania.ConfigDB.bAutoPlay.FT == true && CDTXMania.ConfigDB.bAutoPlay.CY == true &&
                        CDTXMania.ConfigDB.bAutoPlay.LP == true && CDTXMania.ConfigDB.bAutoPlay.LBD == true)
            {
                automode = 4;	// 3 Auto
            }
            ...
            #region [ Guitar ]
            // "OFF", "ON" 
            //			l.Add( ( CDTXMania.ConfigDB.bAutoPlay.Guitar == true ) ? 1 : 0 );
            //			l.Add( ( CDTXMania.ConfigDB.bAutoPlay.Bass   == true ) ? 1 : 0 );
            if (CDTXMania.ConfigDB.bギターが全部オートプレイである)
            {
                automode = 0;	// All Auto
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.GtR == true && CDTXMania.ConfigDB.bAutoPlay.GtG == true &&
                        CDTXMania.ConfigDB.bAutoPlay.GtB == true && CDTXMania.ConfigDB.bAutoPlay.GtY == true && CDTXMania.ConfigDB.bAutoPlay.GtP == true && CDTXMania.ConfigDB.bAutoPlay.GtPick == false &&
                        CDTXMania.ConfigDB.bAutoPlay.GtW == false)
            {
                automode = 1;	// Auto Neck
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.GtR == false && CDTXMania.ConfigDB.bAutoPlay.GtG == false &&
                        CDTXMania.ConfigDB.bAutoPlay.GtB == false && CDTXMania.ConfigDB.bAutoPlay.GtY == false && CDTXMania.ConfigDB.bAutoPlay.GtP == false && CDTXMania.ConfigDB.bAutoPlay.GtPick == true &&
                        CDTXMania.ConfigDB.bAutoPlay.GtW == false)
            {
                automode = 2;	// Auto Pick
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.GtR == false && CDTXMania.ConfigDB.bAutoPlay.GtB == false &&
                        CDTXMania.ConfigDB.bAutoPlay.GtB == false && CDTXMania.ConfigDB.bAutoPlay.GtY == false && CDTXMania.ConfigDB.bAutoPlay.GtP == false && CDTXMania.ConfigDB.bAutoPlay.GtPick == false &&
                        CDTXMania.ConfigDB.bAutoPlay.GtW == false)
            {
                automode = 4;	// OFF
            }
            ...
            else if (CDTXMania.ConfigDB.bAutoPlay.BsR == true && CDTXMania.ConfigDB.bAutoPlay.BsB == true &&
                        CDTXMania.ConfigDB.bAutoPlay.BsB == true && CDTXMania.ConfigDB.bAutoPlay.BsPick == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BsW == false)
            {
                automode = 1;	// Auto Neck
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.BsR == false && CDTXMania.ConfigDB.bAutoPlay.BsB == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BsB == false && CDTXMania.ConfigDB.bAutoPlay.BsPick == true &&
                        CDTXMania.ConfigDB.bAutoPlay.BsW == false)
            {
                automode = 2;	// Auto Pick
            }
            else if (CDTXMania.ConfigDB.bAutoPlay.BsR == false && CDTXMania.ConfigDB.bAutoPlay.BsB == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BsB == false && CDTXMania.ConfigDB.bAutoPlay.BsPick == false &&
                        CDTXMania.ConfigDB.bAutoPlay.BsW == false)
            {
                automode = 4;	// OFF
            }
        ...
        public override void tEnter押下Main(int nSortOrder)
        {
            switch (n現在の選択行)
            {
                ...
                case (int)EOrder.ScrollSpeed:
                    CDTXMania.ConfigDB.n譜面スクロール速度[nCurrentTarget] = (int)GetObj現在値((int)EOrder.ScrollSpeed);
                    break;

                case (int)EOrder.Dark:
                    {
                        Eダークモード d = (Eダークモード)GetIndex((int)EOrder.Dark);
                        CDTXMania.ConfigDB.eDark = d;
                        SetValueToAllTarget((int)EOrder.Dark, (int)d);		// 全楽器で共有する値のため、全targetに値を展開する

                        if (d == Eダークモード.FULL)
                        {
                            CDTXMania.ConfigDB.nLaneDisp[nCurrentTarget] = 3;
                            CDTXMania.ConfigDB.bJudgeLineDisp[nCurrentTarget] = false;
                            CDTXMania.ConfigDB.bLaneFlush[nCurrentTarget] = false;
                        }
                        else if (d == Eダークモード.HALF)
                        {
                            CDTXMania.ConfigDB.nLaneDisp[nCurrentTarget] = 1;
                            CDTXMania.ConfigDB.bJudgeLineDisp[nCurrentTarget] = true;
                            CDTXMania.ConfigDB.bLaneFlush[nCurrentTarget] = true;
                        }
                        else
                        {
                            CDTXMania.ConfigDB.nLaneDisp[nCurrentTarget] = 0;
                            CDTXMania.ConfigDB.bJudgeLineDisp[nCurrentTarget] = true;
                            CDTXMania.ConfigDB.bLaneFlush[nCurrentTarget] = true;
                        }
                    }
                ...
                case (int)EOrder.Risky:
                    {
                        ...
                        CDTXMania.ConfigDB.nRisky = r;
                        SetValueToAllTarget((int)EOrder.Risky, r);			// 全楽器で共有する値のため、全targetに値を展開する
                    }
                    break;
                case (int)EOrder.PlaySpeed:
                    CDTXMania.ConfigDB.n演奏速度 = (int)GetObj現在値((int)EOrder.PlaySpeed);
                    break;
                case (int)EOrder.SuddenHidden:
                    {
                        CDTXMania.ConfigDB.nHidSud[nCurrentTarget] = (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] + 1) % 5;

                        if (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] == 0)
                        {
                            CDTXMania.ConfigDB.bHidden[nCurrentTarget] = false;
                            CDTXMania.ConfigDB.bSudden[nCurrentTarget] = false;
                        }
                        else if (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] == 1)
                        {
                            CDTXMania.ConfigDB.bHidden[nCurrentTarget] = true;
                            CDTXMania.ConfigDB.bSudden[nCurrentTarget] = false;
                        }
                        else if (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] == 2)
                        {
                            CDTXMania.ConfigDB.bHidden[nCurrentTarget] = false;
                            CDTXMania.ConfigDB.bSudden[nCurrentTarget] = true;
                        }
                        else if (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] == 3)
                        {
                            CDTXMania.ConfigDB.bHidden[nCurrentTarget] = true;
                            CDTXMania.ConfigDB.bSudden[nCurrentTarget] = true;
                        }
                        else if (CDTXMania.ConfigDB.nHidSud[nCurrentTarget] == 4)
                        {
                            CDTXMania.ConfigDB.bHidden[nCurrentTarget] = true;
                            CDTXMania.ConfigDB.bSudden[nCurrentTarget] = true;
                        }
                    }
                    //CDTXMania.ConfigDB.nHidSud = (int) GetObj現在値((int) EOrder.SuddenHidden);
                    break;
                case (int) EOrder.AutoGhost: // #35411 chnmr0 AUTOゴーストデータ
                    EAutoGhostData gd = (EAutoGhostData)GetIndex((int)EOrder.AutoGhost);
                    CDTXMania.ConfigDB.eAutoGhost[ nCurrentTarget ] = gd;
                    break;
                ...
                case (int)EOrder.TargetGhost: // #35411 chnmr0 ターゲットゴーストデータ
                    ...
                    CDTXMania.ConfigDB.eTargetGhost[ nCurrentTarget ] = gtd;
                    break;
        ...
        private void SetAutoParameters()
        {
            for (int target = 0; target < 3; target++)
            {
                ...
                {
                    CDTXMania.ConfigDB.bAutoPlay[i + start] = (str[i] == 'A') ? true : false;
                }
            }
        }
                ...
                case (int)E楽器パート.DRUMS:
                    switch (lci[nCurrentConfigSet][target][(int)EOrder.AutoMode].GetIndex())
                    {
                        ...
                        case 5:	// Custom
                            for (int i = 0; i < 11; i++)
                            {
                                s += (CDTXMania.ConfigDB.bAutoPlay[i]) ? "A" : "_";
                            }
                            break;
                        case 6:	// OFF
                            s = "___________";
