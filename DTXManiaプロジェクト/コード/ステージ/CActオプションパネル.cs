        ...
        public override int On進行描画()
        {
            if (!base.bNotActivated)
            {
                ...
                if (this.txオプションパネル != null)
                {
                    ...
                    this.txオプションパネル.tDraw2D(device, 0x2e2, 50, this.rc譜面スピード[drums]);
                    this.txオプションパネル.tDraw2D(device, 0x312, 14, this.rcHS[(configIni.bHidden.Drums ? 1 : 0) + (configIni.bSudden.Drums ? 2 : 0)]);
                    this.txオプションパネル.tDraw2D(device, 0x312, 0x20, this.rcHS[(configIni.bHidden.Guitar ? 1 : 0) + (configIni.bSudden.Guitar ? 2 : 0)]);
                    this.txオプションパネル.tDraw2D(device, 0x312, 50, this.rcHS[(configIni.bHidden.Bass ? 1 : 0) + (configIni.bSudden.Bass ? 2 : 0)]);
                    this.txオプションパネル.tDraw2D(device, 0x342, 14, this.rcDark[(int)configIni.eDark]);
                    this.txオプションパネル.tDraw2D(device, 0x342, 0x20, this.rcDark[(int)configIni.eDark]);
                    this.txオプションパネル.tDraw2D(device, 0x342, 50, this.rcDark[(int)configIni.eDark]);
                    this.txオプションパネル.tDraw2D(device, 0x372, 14, this.rcReverse[configIni.bReverse.Drums ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x372, 0x20, this.rcReverse[configIni.bReverse.Guitar ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x372, 50, this.rcReverse[configIni.bReverse.Bass ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 930, 14, this.rcPosition[(int)configIni.判定文字表示位置.Drums]);
                    this.txオプションパネル.tDraw2D(device, 930, 0x20, this.rcPosition[(int)configIni.判定文字表示位置.Guitar]);
                    this.txオプションパネル.tDraw2D(device, 930, 50, this.rcPosition[(int)configIni.判定文字表示位置.Bass]);
                    this.txオプションパネル.tDraw2D(device, 0x3d2, 14, this.rcTight[configIni.bTight ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x3d2, 0x20, this.rcRandom[(int)configIni.eRandom.Guitar]);
                    this.txオプションパネル.tDraw2D(device, 0x3d2, 50, this.rcRandom[(int)configIni.eRandom.Bass]);
                    this.txオプションパネル.tDraw2D(device, 0x402, 14, this.rcComboPos[(int)configIni.ドラムコンボ文字の表示位置]);
                    this.txオプションパネル.tDraw2D(device, 0x402, 0x20, this.rcLight[configIni.bLight.Guitar ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x402, 50, this.rcLight[configIni.bLight.Bass ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x432, 0x20, this.rcLeft[configIni.bLeft.Guitar ? 1 : 0]);
                    this.txオプションパネル.tDraw2D(device, 0x432, 50, this.rcLeft[configIni.bLeft.Bass ? 1 : 0]);
                }
            }
            return 0;
        }
