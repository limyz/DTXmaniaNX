...
namespace DTXMania
{
	...
	/// <para>(2) DTXMania の終了処理の最初（Thank you for playing が消えた直後）に一度だけ、OnUnmanagedReleaseResources()_OnManagedリソースの解放()_On終了() をこの順番で1回ずつ呼び出します。</para>
	/// <para>(3) DTXMania の起動中、すなわち(1)～(2)の間は、どんなシーンであっても、常に OnUpdateAndDraw() を1フレームにつき1回ずつ呼び出します。</para>
	/// <para>(4) Direct3D デバイスのリセット時には、デバイスのリセット前に OnUnmanagedReleaseResources() を1回呼び出し、デバイスをリセットしたのち、OnUnmanagedCreateResources() を1回呼び出します。</para>
	/// <para>(5) Direct3D デバイスのロスト時には、デバイスの再生成前に OnUnmanagedReleaseResources()_OnManagedリソースの解放() を1回ずつ呼び出し、デバイスを再生成したのち、OnManagedCreateResources()_OnUnmanagedリソースの作成() を1回ずつ呼び出します。</para>
	/// </summary>
