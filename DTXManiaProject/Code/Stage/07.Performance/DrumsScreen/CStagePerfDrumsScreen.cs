...
namespace DTXMania
{
	internal class CStagePerfDrumsScreen : CStagePerfCommonScreen
	{
		...
		public CStagePerfDrumsScreen()
		{
			...
			base.listChildActivities.Add( this.actStatusPanels = new CActPerfDrumsStatusPanel() );
			base.listChildActivities.Add( this.act譜面スクロール速度 = new CActPerfScrollSpeed() );
		...
		public override int OnUpdateAndDraw()
		{
			...
			if( !base.bNotActivated )
            {
                ...
                if (base.bJustStartedUpdate)
                {
                    ...
                    this.ctコンボ動作タイマ = new CCounter( 1.0, 16.0, ((60.0 / (CDTXMania.stagePerfDrumsScreen.actPlayInfo.dbBPM) / 16)), CSoundManager.rc演奏用タイマ);
...
//                  this.t進行描画_パネル文字列();
        ...
        public bool bサビ区間;
        public bool bボーナス;
		private readonly EPad[] eチャンネルtoパッド = new EPad[12]
		{
...
//		private CTexture txレーンフレームGB;
		...
		{
			if( ( ( CDTXMania.ConfigIni.eDark != EDarkMode.HALF ) && ( CDTXMania.ConfigIni.eDark != EDarkMode.FULL ) ) && CDTXMania.ConfigIni.bGuitar有効 )
			{
		...
		{
			if ( ( CDTXMania.ConfigIni.eDark != EDarkMode.FULL ) && CDTXMania.ConfigIni.bGuitar有効 )
			{
				if ( CDTXMania.DTX.bチップがある.Guitar )
				{
		...
		protected override void t進行描画_チップ_小節線( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			...
			if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
			{
				...
				if ( configIni.b演奏情報を表示する && ( configIni.nLaneDisp.Drums == 0 || configIni.nLaneDisp.Drums == 1 ) )
                {
                        ...
                        CDTXMania.act文字コンソール.tPrint(858, configIni.bReverse.Drums ? ((base.nJudgeLinePosY.Drums + pChip.nバーからの距離dot.Drums) - 0x11) : ((base.nJudgeLinePosY.Drums - pChip.nバーからの距離dot.Drums) - 0x11), CCharacterConsole.Eフォント種別.白, n小節番号.ToString());
				}
            ...
            /*
			if ( ( pChip.b可視 && configIni.bGuitar有効 ) && ( configIni.eDark != EDarkMode.FULL ) )
			{
