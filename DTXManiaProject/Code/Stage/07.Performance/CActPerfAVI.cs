...
namespace DTXMania
{
    internal class CActPerfAVI : CActivity
    {
        ...
        {
            if (this.txDShow汎用 != null && (CDTXMania.ConfigIni.bDrums有効 ? CDTXMania.stagePerfDrumsScreen.ct登場用.bReachedEndValue : CDTXMania.stagePerfGuitarScreen.ct登場用.bReachedEndValue))
            {
                #region[ 汎用動画 ]
                if (this.ds汎用 != null)
                {
