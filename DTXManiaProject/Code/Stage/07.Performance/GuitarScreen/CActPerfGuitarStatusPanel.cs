            ...
            if (!base.bNotActivated)
            {
                ...
                for( int i = 1; i < 3; i++ )
                {
                    if (this.n本体X[ i ] != 0)
                    {
                        ...
                        this.t小文字表示( 80 + this.n本体X[ i ], 72 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Perfect ) );
                        this.t小文字表示( 80 + this.n本体X[ i ], 102 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Great ) );
                        this.t小文字表示( 80 + this.n本体X[ i ], 132 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Good ) );
                        this.t小文字表示( 80 + this.n本体X[ i ], 162 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Poor ) );
                        this.t小文字表示( 80 + this.n本体X[ i ], 192 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Miss ) );
                        this.t小文字表示( 80 + this.n本体X[ i ], 222 + this.n本体Y, string.Format( "{0,4:###0}", CDTXMania.stagePerfGuitarScreen.actCombo.n現在のコンボ数.最高値[ i ] ) );

                        int n現在のノーツ数 =
                            CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含む[ i ].Perfect +
                            CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含む[ i ].Great +
                            CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含む[ i ].Good +
                            CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含む[ i ].Poor +
                            CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含む[ i ].Miss;

                        dbPERFECT率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Perfect) / n現在のノーツ数 );
                        dbGREAT率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Great / n現在のノーツ数 ) );
                        dbGOOD率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Good / n現在のノーツ数 ) );
                        dbPOOR率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Poor / n現在のノーツ数 ) );
                        dbMISS率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.nヒット数_Auto含まない[ i ].Miss / n現在のノーツ数 ) );
                        dbMAXCOMBO率 = Math.Round( ( 100.0 * CDTXMania.stagePerfGuitarScreen.actCombo.n現在のコンボ数.最高値[ i ] / n現在のノーツ数 ) );
                        ...
                        //this.t大文字表示(58 + this.n本体X[ i ], 277 + this.n本体Y, string.Format( "{0,6:##0.00}", CDTXMania.stagePerfGuitarScreen.actStatusPanels.db現在の達成率.Guitar ) );
                        if (this.txSkillMax != null && CDTXMania.stagePerfDrumsScreen.actStatusPanels.db現在の達成率.Guitar >= 100.0)
                        {
                            this.txSkillMax.tDraw2D(CDTXMania.app.Device, 127 + this.n本体X[i], 277 + this.n本体Y);
                        }
                        else
                        {
                            this.t大文字表示(58 + this.n本体X[i], 277 + this.n本体Y, string.Format("{0,6:##0.00}", CDTXMania.stagePerfDrumsScreen.actStatusPanels.db現在の達成率.Guitar));
                        ...
                        if (bCLASSIC)
                        {
                            this.t大文字表示(88 + this.n本体X[i], 363 + this.n本体Y, string.Format("{0,6:##0.00}", CDTXMania.stagePerfGuitarScreen.actStatusPanels.db現在の達成率.Guitar * (CDTXMania.DTX.LEVEL[i]) * 0.0033));
                        }
                        else
                        {
                            this.t大文字表示(88 + this.n本体X[i], 363 + this.n本体Y, string.Format("{0,6:##0.00}", CDTXMania.stagePerfGuitarScreen.actStatusPanels.db現在の達成率.Guitar * (CDTXMania.DTX.LEVEL[i] / 10.0) * 0.2));
                        }
