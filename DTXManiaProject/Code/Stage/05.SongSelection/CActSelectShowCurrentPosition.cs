	...
	internal class CActSelectShowCurrentPosition : CActivity
	{
		...
		public override int OnUpdateAndDraw()
		{
			...
			#region [ スクロールバーの描画 #27648 ]
                this.txScrollBar.tDraw2D(CDTXMania.app.Device, x - (CDTXMania.stageSongSelection.ct登場時アニメ用共通.n現在の値 / 2f), y, new Rectangle(0, 0, 12, 492));	// 本当のy座標は88なんだが、なぜか約30のバイアスが掛かる___
			#endregion
			#region [ スクロール地点の描画 (計算はCActSelect曲リストで行う。スクロール位置と選曲項目の同期のため。)#27648 ]
                ...
                if ( py <= 492 - 12 && py >= 0 )
				{
                    this.txScrollBar.tDraw2D(CDTXMania.app.Device, x - (CDTXMania.stageSongSelection.ct登場時アニメ用共通.n現在の値 / 2f), y + py, new Rectangle(0, 492, 12, 12));
				}
			#endregion
			}
