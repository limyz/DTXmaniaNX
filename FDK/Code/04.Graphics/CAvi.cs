...
namespace FDK
{
	public class CAvi : IDisposable
	{
		...
		public unsafe void tBitmap24ToGraphicsStreamR5G6B5( BitmapUtil.BITMAPINFOHEADER* pBITMAPINFOHEADER, IntPtr dataPointer, int nWidth, int nHeight )
		{
			...
			ushort* pTexture = (ushort*) dataPointer.ToPointer();
		...
		public unsafe void tBitmap24ToGraphicsStreamX8R8G8B8( BitmapUtil.BITMAPINFOHEADER* pBITMAPINFOHEADER, IntPtr dataPointer, int nWidth, int nHeight )
		{
			int nBmpWidth = pBITMAPINFOHEADER->biWidthビットマップの幅dot;
			int nBmpHeight = pBITMAPINFOHEADER->biHeightビットマップの高さdot;
			int nBmpLineByte = ( nBmpWidth * 3 ) + ( ( 4 - ( ( nBmpWidth * 3 ) % 4 ) ) % 4 );
			
			uint* pTexture = (uint*) dataPointer.ToPointer();
			byte* pBitmap = (byte*) ( pBITMAPINFOHEADER + 1 );
